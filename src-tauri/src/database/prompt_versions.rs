...
impl PromptVersionRepository {
    ...
    pub fn delete_template(&self, template_id: i64) -> Result<()> {
        self.with_conn_inner(|conn| {
            ...
            // 先删除模板的所有版本（外键约束可能会自动删除，但这里显式处理）
            let versions_deleted = conn.execute(
                "DELETE FROM prompt_versions WHERE template_id = ?1",
                params![template_id],
            )?;
            ...
            if template_deleted == 0 {
                ...
                return Err(anyhow::anyhow!("模板 {} 不存在或已被删除", template_id).into());
            }
