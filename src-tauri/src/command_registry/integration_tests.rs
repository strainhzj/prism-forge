...
mod tests {
    ...
    use crate::command_registry::{
        CommandInfo, CommandRegistry, CommandStatus, CommandValidator, DiagnosticTool,
        EnhancedErrorHandler, ErrorCategory, InitState, Module, ModuleInitializer,
    };
    use std::sync::Arc;
    ...
    fn test_complete_command_registration_flow() {
        ...
        for (name, deps) in commands {
            ...
            let command =
                CommandInfo::with_dependencies(name.to_string(), deps_vec).mark_registered();
    ...
    fn test_module_initialization_with_command_registration() {
        ...
        initializer
            .register_module(Box::new(MockDatabaseModule {
                initialized: db_initialized.clone(),
            }))
            .unwrap();
        initializer
            .register_module(Box::new(MockMonitorModule {
        ...
        assert!(
            db_pos < monitor_pos,
            "Database should initialize before monitor"
        );
        ...
        assert!(
            *db_initialized.lock().unwrap(),
            "Database should be initialized"
        );
        assert!(
            *monitor_initialized.lock().unwrap(),
            "Monitor should be initialized"
        );

        // Verify module states
        assert!(matches!(
            initializer.get_module_state("database"),
            Some(InitState::Ready)
        ));
        assert!(matches!(
            initializer.get_module_state("monitor"),
            Some(InitState::Ready)
        ));
    }
    ...
    fn test_error_handling_flow() {
        ...
        assert!(
            matches!(category, ErrorCategory::CommandNotFound),
        ...
        assert!(
            matches!(category, ErrorCategory::ModuleInitializationFailed),
            "Expected ModuleInitializationFailed, got {:?}",
            category
        );
    }
    ...
    fn test_diagnostic_tool_integration() {
        ...
        let diagnostic = DiagnosticTool::new(Arc::new(registry), Arc::new(initializer));
        ...
        assert!(report
            .registered_commands
            .contains(&"valid_command".to_string()));
        assert!(report
            .registered_commands
            .contains(&"dependent_command".to_string()));
    ...
    fn test_end_to_end_command_call_simulation() {
        ...
        for call in test_calls {
            ...
            assert!(
                detailed_status.call_count > 0,
                "Call count should be incremented"
            );
            assert!(
                detailed_status.last_called.is_some(),
                "Last called should be set"
            );
        }
    ...
    fn test_regression_command_registration() {
        ...
        for name in &known_commands {
            assert!(
                registry.has_command(name),
                "Command {} should be registered",
                name
            );
            assert!(
                matches!(
                    registry.get_command_status(name),
                    Some(CommandStatus::Registered)
                ),
                "Command {} should have Registered status",
                name
            );
        }
        ...
        for name in &known_commands {
            assert!(
                ...
                "Command {} should be in available list",
                name
            );
        }
    }
    ...
    fn test_dependency_chain_validation() {
        ...
        let cmd_b =
            CommandInfo::with_dependencies("command_b".to_string(), vec!["command_c".to_string()])
                .mark_registered();
        let cmd_a =
            CommandInfo::with_dependencies("command_a".to_string(), vec!["command_b".to_string()])
                .mark_registered();
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.message.contains("nonexistent_dependency")),
    ...
    fn test_health_check_integration() {
        ...
        let mut initializer = ModuleInitializer::new();
        initializer
            .register_module(Box::new(HealthyModule))
            .unwrap();
        initializer
            .register_module(Box::new(UnhealthyModule))
            .unwrap();
    ...
    fn test_alert_mechanism_integration() {
        ...
        let disabled_cmd =
            CommandInfo::new("disabled_command".to_string()).with_status(CommandStatus::Disabled);
    ...
    fn test_full_system_startup_simulation() {
        ...
        assert!(
            parser_pos < optimizer_pos,
            "parser should init before optimizer"
        );
        ...
        for (name, deps) in commands {
            let deps_vec: Vec<String> = deps.iter().map(|s: &&str| s.to_string()).collect();
            let cmd = CommandInfo::with_dependencies(name.to_string(), deps_vec).mark_registered();
            registry.register_command(cmd).unwrap();
        }
        ...
        for module_name in ["database", "monitor", "llm", "parser", "optimizer"] {
            assert!(
                matches!(
                    initializer.get_module_state(module_name),
                    Some(InitState::Ready)
                ),
...
mod integration_unit_tests {
    ...
    use crate::command_registry::validator::AutoTestConfig;
    use crate::command_registry::{
        CommandInfo, CommandRegistry, CommandStatus, CommandValidator, EnhancedErrorHandler,
    ...
    fn test_call_chain_tracking_correctness() {
        ...
        let initial_status = registry
            .get_command_status_detailed("test_command")
            .unwrap();
        assert_eq!(
            initial_status.call_count, 0,
            "Initial call count should be 0"
        );
        assert!(
            initial_status.last_called.is_none(),
            "Initial last_called should be None"
        );
        ...
        for i in 1..=5 {
            ...
            assert!(
                status.last_called.is_some(),
                "last_called should be set after call"
            );
        }
        ...
        assert!(
            history.len() >= 6,
            "History should contain registration and all calls"
        );
    }
    ...
    fn test_call_chain_nonexistent_command() {
        ...
        let status = registry
            .get_command_status_detailed("existing_cmd")
            .unwrap();
        assert_eq!(
            status.call_count, 0,
            "Existing command should be unaffected"
        );

        // Verify non-existent command has no status
        assert!(registry
            .get_command_status_detailed("nonexistent_cmd")
            .is_none());
    }
    ...
    fn test_call_chain_rapid_calls() {
        ...
        assert_eq!(
            status.call_count, call_count,
            "All rapid calls should be counted"
        );
    }
    ...
    fn test_regression_detects_missing_commands() {
        ...
        let missing: Vec<_> = known_commands
            .iter()
    ...
    fn test_regression_status_consistency() {
        ...
        let failed_cmd = CommandInfo::new("failed_cmd".to_string())
            .with_status(CommandStatus::Failed("Test failure".to_string()));
        let disabled_cmd =
            CommandInfo::new("disabled_cmd".to_string()).with_status(CommandStatus::Disabled);
    ...
    fn test_regression_dependency_chain_integrity() {
        ...
        assert!(
            errors.is_empty(),
            "Valid dependency chain should have no errors"
        );
        ...
        assert!(
            !errors.is_empty(),
            "Broken dependency chain should have errors"
        );
    }
    ...
    fn test_integration_module_failure_handling() {
        ...
        let mut initializer = ModuleInitializer::new();
        initializer
            .register_module(Box::new(FailingModule))
            .unwrap();
        ...
        assert!(
            result.is_err(),
            "Initialization should fail for failing module"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.message.contains("Intentional failure")
                    || e.message.contains("failing_module")),
