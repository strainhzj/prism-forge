...
use crate::database::models::{
    PromptComponent, PromptParameter, PromptTemplate, PromptVersion, PromptVersionDiff,
};
...
pub async fn cmd_get_prompt_templates() -> Result<Vec<PromptTemplate>, String> {
    ...
    repo.list_templates().map_err(|e| e.to_string())
}

/// 根据名称获取提示词模板
#[tauri::command]
pub async fn cmd_get_prompt_template_by_name(
    name: String,
) -> Result<Option<PromptTemplate>, String> {
    let repo = PromptVersionRepository::from_default_db()
        .map_err(|e| format!("创建版本仓库失败: {}", e))?;

    repo.get_template_by_name(&name).map_err(|e| e.to_string())
}

/// 根据场景获取提示词模板
#[tauri::command]
pub async fn cmd_get_prompt_template_by_scenario(
    scenario: String,
) -> Result<Option<PromptTemplate>, String> {
    let repo = PromptVersionRepository::from_default_db()
        .map_err(|e| format!("创建版本仓库失败: {}", e))?;
...
pub async fn cmd_get_prompt_versions(template_id: i64) -> Result<Vec<PromptVersion>, String> {
    ...
    repo.list_versions(template_id).map_err(|e| e.to_string())
}

/// 获取模板的激活版本
#[tauri::command]
pub async fn cmd_get_active_prompt_version(
    template_id: i64,
...
pub async fn cmd_save_prompt_version(
    ...
    repo.create_and_activate_version(template_id, content, components, parameters, &created_by)
        .map_err(|e| e.to_string())
}
...
pub async fn cmd_migrate_all_template_components(template_id: i64) -> Result<usize, String> {
    ...
    for version in versions {
        if let Some(id) = version.id {
            let count = repo
                .migrate_components_from_content(id)
...
/// 获取版本的所有组件（旧版，兼容保留）
#[tauri::command]
pub async fn cmd_get_prompt_components_by_id(
    version_id: i64,
) -> Result<Vec<PromptComponent>, String> {
    let repo = PromptVersionRepository::from_default_db()
        .map_err(|e| format!("创建版本仓库失败: {}", e))?;

    repo.list_components(version_id).map_err(|e| e.to_string())
}
...
pub async fn cmd_get_prompt_parameters(version_id: i64) -> Result<Vec<PromptParameter>, String> {
    ...
    repo.list_parameters(version_id).map_err(|e| e.to_string())
}
...
pub async fn cmd_get_prompts_unified(
    ...
    repo.list_prompts_unified(scenario.as_deref(), language.as_deref(), search.as_deref())
        .map_err(|e| e.to_string())
}
...
/// 组件化数据的 JSON 对象
#[tauri::command]
pub async fn cmd_get_prompt_components(template_name: String) -> Result<serde_json::Value, String> {
    let repo = PromptVersionRepository::from_default_db()
        .map_err(|e| format!("创建版本仓库失败: {}", e))?;
    ...
    let content_json: serde_json::Value =
        serde_json::from_str(&version.content).map_err(|e| format!("解析组件数据失败: {}", e))?;
...
pub async fn cmd_update_prompt_components(
    ...
    let new_data: serde_json::Value =
        serde_json::from_str(&components_data).map_err(|e| format!("解析组件数据失败: {}", e))?;
    ...
    let merged_data = merge_component_data(&current_data, &new_data, &updated_languages);
    ...
    let new_version = repo
        .create_version_direct(template_id, next_version, merged_content, "user", &now)
...
/// - `false`: 配置文件未更新
#[tauri::command]
pub async fn cmd_check_config_updated(template_name: String) -> Result<bool, String> {
    use crate::database::init_default_prompts::resolve_config_path;
    use crate::optimizer::config::OptimizerConfig;

    // 解析配置文件路径
    let config_path = resolve_config_path().map_err(|e| format!("解析配置路径失败: {}", e))?;
    ...
    let config: OptimizerConfig =
        toml::from_str(&config_content).map_err(|e| format!("解析配置失败: {}", e))?;
    ...
    let v1_version = repo
        .get_version_by_number(template_id, 1)
    ...
    let config_data: serde_json::Value =
        serde_json::from_str(&config_str).map_err(|e| format!("解析配置数据失败: {}", e))?;
    ...
    fn compare_ignore_timestamp(a: &serde_json::Value, b: &serde_json::Value) -> bool {
        match (a, b) {
            (serde_json::Value::Object(a_map), serde_json::Value::Object(b_map)) => {
                ...
                let a_filtered: std::collections::HashMap<_, _> = a_map
                    .iter()
                    .filter(|(k, _)| *k != "last_modified")
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();

                let b_filtered: std::collections::HashMap<_, _> = b_map
                    .iter()
                    .filter(|(k, _)| *k != "last_modified")
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();
